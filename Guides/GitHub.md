...
### One codebase, two remote repos

To maintain a single local repository and synchronizing it with 
- a personal repo already created (primary repo)which has full tracking history
- a lab or team GitHub repo (create a new one; shared repo)

This allows continued development on the personal repo. Both primary and shared repos can be kept synchronized during the period of active collaboration.

- On GitHub: create an empty repo on the lab GitHub space
  - DON'T initialize this new repo with a readme, .gitignore or a license
- At local repo, verify current remote 
  - `git remote -v`
  - example output: 
      `origin git@github.com:<username>/<current_repo>.git` (fetch)
      `origin git@github.com:<username>/<current_repo>.git` (push)
- Add team repo as the second remote
  - `git remote add shared git@github.com:<lab_path>/shiny-app.git
